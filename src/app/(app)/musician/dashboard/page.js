...
export default function MusicianDashboard() {
  ...
  const { user } = useAuth();
  const { stats, events } = useData(); // ✅ Just use data, don't fetch
...
// // Add this to musician dashboard
const { profile, bookings, fetchProfile, fetchBookings } = useData();
...
//     return; // ← CRITICAL: Don't fetch without user!
//   }

//   console.log('🎵 Dashboard checking data for user:', user.id);
...
//     console.log('🎵 No bookings, fetching...');
//     fetchBookings();
//   }
// }, [user, profile, bookings, fetchProfile, fetchBookings]); // ← Add dependencies
...
useEffect(() => {
  ...
  // Only fetch if data is null/empty
  if (!profile) {
  ...
  // ✅ CRITICAL FIX: Only fetch public events once
  useEffect(() => {
    let mounted = true;
