// src/components/social/ChatWindow.js - ONLY FIX THE MOBILE KEYBOARD
"use client";
...
export default function ChatWindow({ conversation, onBack }) {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-gray-900">
      {/* HEADER - Position sticky keeps it visible */}
      <div className="sticky top-0 z-10 flex-shrink-0 flex items-center gap-3 p-3 lg:p-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
        <button onClick={onBack} className="lg:hidden p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition">
                    ...
                    {message.media_url && (
                      <div className="mb-2 rounded-lg overflow-hidden">
                        {message.media_type === 'image' ? (
                          <img src={message.media_url} alt="Message media" className="max-w-full max-h-48 rounded-lg cursor-pointer"
                            onClick={() => window.open(message.media_url, '_blank')} />
                        ) : (
                          <video src={message.media_url} controls className="max-w-full max-h-48 rounded-lg" />
                        )}
                      </div>
                    )}
                    {message.content && (
                      <div className={`px-3 py-2 rounded-2xl ${
                        isOwn ? 'bg-purple-600 text-white rounded-br-none'
      ...
      {/* ERROR */}
      {error && (
        <div className="flex-shrink-0 px-3 py-2 bg-red-50 dark:bg-red-900/20 border-t border-red-200 dark:border-red-800">
      ...
      {/* INPUT - Sticky bottom */}
      <form onSubmit={handleSend} className="sticky bottom-0 flex-shrink-0 p-3 lg:p-4 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
        {mediaPreview && (
          <div className="mb-2 relative inline-block">
            <img src={mediaPreview} alt="Preview" className="max-w-24 max-h-24 rounded-lg border-2 border-purple-200" />
            <button type="button" onClick={removeMedia} className="absolute -top-1 -right-1 p-1 bg-red-500 rounded-full text-white">
              <X className="w-3 h-3" />
            </button>
          </div>
        )}
        <div className="flex items-end gap-2">
          ...
          <textarea ref={textareaRef} value={messageText} onChange={(e) => setMessageText(e.target.value)}
            onKeyDown={handleKeyDown} placeholder="Type a message..." disabled={sending} maxLength={1000} rows={1}
          ...
          <button type="submit" disabled={(!messageText.trim() && !mediaFile) || sending}
            ...
            {sending ? <Loader2 className="w-4 h-4 lg:w-5 lg:h-5 animate-spin" /> : <Send className="w-4 h-4 lg:w-5 lg:h-5" />}
          </button>
        </div>
      </form>
    </div>
