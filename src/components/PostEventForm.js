// src/components/PostEventForm.js
"use client";
...
const handleSubmit = async (e) => {
  ...
  const proposedAmount = form.proposed_amount ? parseInt(form.proposed_amount, 10) : null;
  ...
  try {
    ...
    if (form.media_file) {
      ...
      if (uploadError) {
        throw new Error("Failed to upload file. Check storage policies.");
      }
    ...
    // 2. Insert event details into the database
    ...
    // Check for a database error and handle it
    if (dbError) {
      console.error("Supabase Insert Error:", dbError);
      setError(`Failed to post event. Check database policies. Full error: ${dbError.message}`);
    } else {
      // Only call onSuccess if there was no database error
    ...
    console.log("Finished submission, setting loading to false.");
    setLoading(false);
  }
};
